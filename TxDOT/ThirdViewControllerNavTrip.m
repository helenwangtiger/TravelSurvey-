...
@interface ThirdViewControllerNavTrip ()

@end

@implementation ThirdViewControllerNavTrip
@synthesize topBar,questionTitleTrip,questionDescTrip,answerTrip,pickerChoices,questionNum,questionTotalNum,recordType,questionDes,answerView,preButton,nextButton,text,picker,tripName,sampleNumAlert,startTime, endTime;
...
-(IBAction)submit {
    ...
    NSString *plist = [[NSString alloc] initWithFormat:@"trips.plist"];
    NSString *path = [documentsDirectory stringByAppendingPathComponent:plist];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if (![fileManager fileExistsAtPath: path]) {
        path = [documentsDirectory stringByAppendingPathComponent:plist];
    }
    ...
    NSString *tripPlist = [[NSString alloc] initWithFormat:@"%@.plist",tripName];
    NSString *path2 = [documentsDirectory stringByAppendingPathComponent:tripPlist];
    NSMutableDictionary *dataSubmit = [[NSMutableDictionary alloc] initWithContentsOfFile:path2];
    NSString *sampleNumber = [[NSUserDefaults standardUserDefaults]stringForKey:@"sample_number"];
    NSLog(@"samplenumber in trip is %@",sampleNumber);
    NSString *coordinateString = [[NSString alloc] init];
    for(int i=0;i<[tripData count];i++) {
        NSString *newString = [NSString stringWithFormat:@"%@|%@|nostamp|",tripData[i][0],tripData[i][1]];
        coordinateString = [NSString stringWithFormat:@"%@%@",coordinateString,newString];
    }
            ...
            NSLog(@"%@",serverAddress);
            NSLog(@"%@",error);
            [dataSubmit setObject:@"insertTrip" forKey:@"actionType"];
            [dataSubmit setObject:tripName forKey:@"trip_name"];
            [dataSubmit setObject:sampleNumber forKey:@"sample_number"];
            HttpSyncCommunication* tripHttp = [HttpSyncCommunication singleHttpSync];
            [tripHttp sendHTTPRequest:dataSubmit];
            NSString *status = [[NSUserDefaults standardUserDefaults]stringForKey:tripName];
            NSLog(@"status before set 1 is %@",status);
            status = @"1";
            [[NSUserDefaults standardUserDefaults] setValue:status forKey:tripName];
...
-(void)saveToDictionary {
    ...
    NSInteger row;
    if([answerTrip[questionInt][0] isEqualToString:@"1"] || [answerTrip[questionInt][0] isEqualToString:@"3"]) {
        if(text.text==NULL||text.text==nil) {
            value = @"";
        }
        else {
            value = text.text;
        ...
        rowKey = [NSString stringWithFormat:@"%@%@",answerTrip[questionInt][2],@"rowKey"];
        NSLog(@"%ld",(long)row);
    }
...
-(void)readFromDictionary {
    ...
    if([answerTrip[questionInt][0] isEqualToString:@"1"]||[answerTrip[questionInt][0] isEqualToString:@"3"]) {
        ...
        NSString *value = [[NSString alloc] init];
        NSString *rowKey = [[NSString alloc] initWithFormat:@"%@%@",answerTrip[questionInt][2],@"rowKey"];
...
-(void)questionIntChanged {
    ...
    questionTotalNum.text = [NSString stringWithFormat:@"%u",[questionTitleTrip count]-1];
...
-(void)generateAnswers {
    ...
    if([answerTrip[questionInt][0] isEqualToString:@"1"]) {
        text = [[UITextField alloc] initWithFrame:CGRectMake(24, 15, 250, 35)];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if (screensize.height<500) {
        [preButton setFrame:CGRectMake(20, 400, 89, 30)];
        [nextButton setFrame:CGRectMake(210, 400, 89, 30)];
        [answerView setFrame:CGRectMake(10, 180, 298, 180)];
        [topBar setFrame:CGRectMake(0, 0, 320, 44)];
        
        
    }
    if (screensize.height >500) {
        [answerView setFrame:CGRectMake(10,200,298,180)];
        [preButton setFrame:CGRectMake(20, 420, 89, 40)];
        [nextButton setFrame:CGRectMake(210, 420, 89, 40)];
    }
...
{
    [super viewDidLoad];
    topBar.topItem.title = [[NSString alloc] initWithFormat:@"Trip Info: %@", tripName];
    ...
    [answerChoices addObject:@"Social/Recreational"];
    ...
    [answerChoices addObject:@"Shopping Mall/Department Store"];
    [answerChoices addObject:@"Convenience Store/Gas Station"];
    ...
    [answerChoices addObject:@"Motorcycle/Moped"];
    [answerChoices addObject:@"Other"];
    [answerChoices addObject:@"Don't Know"];
    [answerChoices addObject:@"Prefer not to answer"];
    [answerContent addObject:@"2"];
